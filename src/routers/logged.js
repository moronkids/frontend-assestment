...
const Logged = ({ component: Component, ...rest }) => {
  const { isLoading, isError, data, error, mutate } = useMutation(
    ...
    const { isLoading:loadDec, isError: isErrorDec, data: dataDec, error: errDec, mutate: mutateDec } = useMutation(
      ...
      const { isLoading:loadDel, isError: isErrorDel, data: dataDel, error: errDel, mutate: mutateDel } = useMutation(
        "deleteTrx",
        async (e) => deleteTrx({"id_transaksi" : 3}),
        {}
        );
        const {details, setDetails, confirmation, setConfirmation, id, action, setPopUpRate, popUpRate} = useContext(Hooks);
    ...
    <Route
      ...
      render={props => {
        return (
          <>
                 <Popup />
                      <div
            onClick={() => {setDetails(!details); setPopUpRate(!popUpRate)}}
      ...
      >
        ...
        <DialogActions>
          ...
          <Button onClick={(e) => {setConfirmation(!confirmation); setDetails(false); hitConfirmation(action) }} autoFocus>
            Agree
          </Button>
        </DialogActions>
      </Dialog>
